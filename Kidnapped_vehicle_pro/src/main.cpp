...
int main() {
	...
	ofstream* fout = new ofstream("../data/output.txt", ios::app);
	cout<<"Start Run Partical Filter......."<<endl;
    ...
    //for (int i = 0; i < 2; ++i) {
		...
		(*fout) << "Mean error: x " << cum_mean_error[0] << " y " << cum_mean_error[1] << " yaw " << cum_mean_error[2] << endl;
	...
	if (runtime < max_runtime && pf.initialized()) {
		cout << "Success! The Result In output.txt file" << endl;
	}
	else if (!pf.initialized()) {
		cout << "Please initialized your filter." << endl;
	}
	else {
		cout << "Runtime " << runtime << "larger than the maximum allowable runtime, " << max_runtime << endl;
		return -1;
	}
